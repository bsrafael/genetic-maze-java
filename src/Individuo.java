...
public class Individuo {
  ...
  // gera o valor de aptidão, será calculada pelo número de bits do gene iguais ao
  // da solução
  private void geraAptidao() {
    ...
    for (int i = 0; i < geneBits.length; i++) {
      if (geneBits[i].compareTo("00") == 0)
        directions[i] = Maze.RIGHT;
      if (geneBits[i].compareTo("01") == 0)
        directions[i] = Maze.UP;
      if (geneBits[i].compareTo("10") == 0)
        directions[i] = Maze.LEFT;
      if (geneBits[i].compareTo("11") == 0)
        directions[i] = Maze.DOWN;
    }
    ...
    for (int i = 0; i < directions.length; i++) {
      ...
      if (!Arrays.asList(currentCell.actions).contains(directions[i])) {
        ...
        if (directions[i].compareTo(Maze.UP) == 0 && currentY + 1 <= 3)
          currentY += 1;
        if (directions[i].compareTo(Maze.DOWN) == 0 && currentY - 1 >= 0)
          currentY -= 1;
        if (directions[i].compareTo(Maze.RIGHT) == 0 && currentX + 1 <= 3)
          currentX += 1;
        if (directions[i].compareTo(Maze.LEFT) == 0 && currentX - 1 >= 0)
          currentX -= 1;
    ...
    // deve usar a solução fixa fornecida?
    if (Algoritmo.getUsePredefinedSolution()) {
